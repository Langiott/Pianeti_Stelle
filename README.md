...
### Setup per lavorare con DESI-EDR

1. **Jupyter Notebook** per lavoro locale interattivo
2. **Ambiente virtuale** (`env`) con librerie scientifiche (simile a conda)
3. **PostgreSQL** per la gestione del database DESI
4. **Driver Python**: `sqlalchemy` + `psycopg2` per connettersi a PostgreSQL da Jupyter
5. **Repository specprod-db**:
   ```bash
   git clone https://github.com/desihub/specprod-db
   pip install .
   ```
...
```bash
psql -U your_username -l                    # Elenca database
createdb -U your_username specprod-db       # Crea database
```

Collegamento da Python:
```python
...
### Verifica servizio PostgreSQL su Windows

1. Premi `Win + R`, digita `services.msc`
2. Trova il servizio `postgresql-x64-15.8-2` (o versione installata)
3. Verifica che sia in esecuzione, altrimenti avvia

Risorse ufficiali:
- [Accesso DESI-EDR](https://help.desi.lbl.gov/index.php?qa=8&qa_1=accessing-desi-edr-redshift-and-spectra-using-list-of-and-dec)
- [DESI Hub](https://desi.lbl.gov/)
- [Documentazione specprod-db](https://specprod-db.readthedocs.io/)
